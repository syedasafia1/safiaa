...
class Manager:
    ...
    def createProgram(cls):
        try:
            mycursor.execute("CREATE TABLE ParticipantManager (name VARCHAR(30), age INT, address VARCHAR(30))")
    ...
    def insertProgram(cls):
        try:
            mycursor.execute("INSERT INTO ParticipantManager (name, age, address) VALUES ('misbha', 34, 'cowlbazar')")
            mydb.commit()
    ...
    def updateProgram(cls):
        try:
            mycursor.execute("UPDATE ParticipantManager SET age=19 WHERE name='s'")
            mydb.commit()
    ...
    def deleteProgram(cls):
        try:
            mycursor.execute("DELETE FROM ParticipantManager WHERE name='safiya'")
            mydb.commit()
